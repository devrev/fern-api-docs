...
It is started by sending the worker a message with event of type `EXTRACTION_DATA_DELETE`.

### Snap-in response

The worker must respond to Airdrop with a message with event of type `EXTRACTION_DATA_DELETE_DONE` when done or
...
It is started by sending the span-in a message with event of type `EXTRACTION_ATTACHMENTS_DELETE`.

### Snap-in response

The snap-in must respond to Airdrop with a message with event of type `EXTRACTION_ATTACHMENTS_DELETE_DONE` when done,
