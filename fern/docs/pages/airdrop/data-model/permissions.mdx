...
For `scope=external` articles the expected behavior is they will be published to Portal/PLuG and shared with customers.
...
<Info>
Currently, only 1-way sync (from the external system to DevRev) is supported for authorization policy.
</Info>
