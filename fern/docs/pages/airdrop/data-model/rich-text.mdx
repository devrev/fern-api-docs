...
| `ref_type`             | String | Yes      | Type of the mentioned item (e.g., "issue", "comment"). The recipe converts this based on user mappings.                              |
...
Article permissions are managed through the `shared_with` field, which can reference users, groups, and platform groups. Refer to the [permissions](./permissions.mdx) for more details.
...
The platform will replace the mention with the corresponding artifact ID.
