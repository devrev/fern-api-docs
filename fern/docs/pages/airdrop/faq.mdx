...
<AccordionGroup>
  <Accordion title="Tips for defining record types">
    ...
    but in other cases, external systems can have configurable custom types or subtypes (for example, issuetypes in Jira).
  ...
  <Accordion title="Handling record IDs and metadata">
    The primary key (id) of the record in the external system doesn't need to be declared as a field in the record type. 
    Instead, id, created_date, and modified_date should be provided at the top level, with all other fields inside the data field. 
    Example:
  ...
  <Accordion title="Custom fields">
    Any fields in the metadata not otherwise mapped are offered to the end user as custom fields, with no additional effort needed.
  </Accordion>
</AccordionGroup>
