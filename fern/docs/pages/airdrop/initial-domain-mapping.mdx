...
It's also recommended that you have the DevRev CLI authenticated appropriately. 
This can be done either with `devrev profiles authenticate -e prod -o <your org slug> -u <your email>` 
...
Obtain a PAT-token from the Settings -> Account tab of the DevOrg where you deploy your snap-in, 
...
This prints the list of Airdrop imports in the org. Select the one you like by running
```bash
...
where the options are: `dvrv-us-1`, `dvrv-eu-1`, `dvrv-in-1`, and `dvrv-de-1`.
...
<Tip>
You can also provide a local metadata file to the command using the `-m` flag, for example: `chef-cli configure-mappings --env prod -m metadata.json`. 
