...
<Steps>
  ...
  ### Declare fields for each record type
    ...
    - `rich_text`: Formatted text with mentions and images. See [rich text](../airdrop/data-model/rich-text.mdx) for more details.
    ...
    - `timestamp`
    - `struct`
    - `permission`: Used in article `shared_with` field. See [permissions](../airdrop/data-model/permissions.mdx) for more details.
    - `type_key`: Used to map permissions to record types. See [permissions](../airdrop/data-model/permissions.mdx) for more details.
