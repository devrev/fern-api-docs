...
sequenceDiagram
    ...
    AirSync->>Snap-in: EXTRACTION_ATTACHMENTS_START
    
    alt Success path
        Snap-in->>ExternalSystem: Request attachments
        ExternalSystem->>Snap-in: Return attachments
        Snap-in->>AirSync: EXTRACTION_ATTACHMENTS_DONE
    
    else Runtime limit reached
        Snap-in->>AirSync: EXTRACTION_ATTACHMENTS_PROGRESS
        AirSync->>Snap-in: EXTRACTION_ATTACHMENTS_CONTINUE
        Note over Snap-in,AirSync: Process continues where it left off
    
    else Rate limiting required
        Snap-in->>AirSync: EXTRACTION_ATTACHMENTS_DELAY (with back-off time)
        Note over AirSync: Waits for specified time
        AirSync->>Snap-in: EXTRACTION_ATTACHMENTS_CONTINUE
        Note over Snap-in,AirSync: Process resumes after delay
    
    else Error occurs
        Snap-in->>AirSync: EXTRACTION_ATTACHMENTS_ERROR
        Note over Snap-in,AirSync: Process terminates
    end
```
...
|-------|-----------|-------------|
| `EXTRACTION_ATTACHMENTS_START` | AirSync → Snap-in | Initiates the attachments extraction |
| `EXTRACTION_ATTACHMENTS_PROGRESS` | Snap-in → AirSync | Indicates process is ongoing but runtime limit (13 minutes) reached |
| `EXTRACTION_ATTACHMENTS_DELAY` | Snap-in → AirSync | Requests a delay due to rate limiting from external system |
| `EXTRACTION_ATTACHMENTS_CONTINUE` | AirSync → Snap-in | Resumes the extraction process after progress update or delay |
| `EXTRACTION_ATTACHMENTS_DONE` | Snap-in → AirSync | Signals successful completion of attachments extraction |
| `EXTRACTION_ATTACHMENTS_ERROR` | Snap-in → AirSync | Indicates that an error occurred during extraction |
