---
title: Data and attachments deletion
---
...
AirSync initiates the data deletion phase when the import is deleted in the DevRev app.
It is started by sending the worker a message with an event of type `EXTRACTION_DATA_DELETE`.

### Response from the snap-in

The worker must respond to AirSync with a message with an event of type `EXTRACTION_DATA_DELETE_DONE`
...
AirSync initiates the attachments deletion phase when an import is deleted from the DevRev app,
...
The snap-in must respond to AirSync with a message with an event of type`EXTRACTION_ATTACHMENTS_DELETE_DONE`
