...
<Info>
Apart from `object_access`, at least one of the `users` or `groups` fields should be mapped and provided in each extracted file.
If you wish to provide the `field_access` field, then you must ensure that the record types and record type categories referenced are also provided in the `object_access` field.
</Info>
