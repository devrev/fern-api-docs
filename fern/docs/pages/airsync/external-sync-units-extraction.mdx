An _external sync unit_ refers to a single unit in the external system that is being AirSynced to DevRev.
...
AirSync starts the external sync unit extraction by sending a message with the event type `EXTRACTION_EXTERNAL_SYNC_UNITS_START`.

The snap-in must reply to AirSync with an `EXTRACTION_EXTERNAL_SYNC_UNITS_DONE` message when finished, 
...
To test your changes, start a new airsync in the DevRev App. If external sync units extraction is successful, you should be prompted to choose an external sync unit from the list.
