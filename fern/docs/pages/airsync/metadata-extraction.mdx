...
`external_domain_metadata.json` file to AirSync.
...
You can check which object types for AirSync DevRev supports [here](/public/airsync/supported-object-types).
...
<Steps>
  ...
  ### Declare the custom record types
    ...
    and you want to sync those too, you have to declare them in the metadata at runtime.
  ...
  ### Categorize external record types
    ...
    This will allow AirSync to load the record types to the external system.
  ...
  ### Declare fields for each record type
    ...
    - `rich_text`: Formatted text with mentions and images. See the [rich text section](../airsync/data-model/rich-text-fields.mdx) for more details.
    ...
    - `timestamp`
    - `struct`
    - `permission`: Used in article `shared_with` field. See the [permissions section](../airsync/data-model/permissions.mdx) for more details.
    - `type_key`: Used to map permissions to record types. See the [permissions section](../airsync/data-model/permissions.mdx) for more details.
