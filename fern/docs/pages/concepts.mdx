...
A [keyring](/snapin-development/references/keyrings/introduction) is a collection of authentication information for external systems. This includes the key (such as a PAT or an API key), its type, the organization ID for which a key is valid,
