...
The `rev-users.create` API is used to create [contacts](https://docs.devrev.ai/product/grow#-contact) which represents an individual user associated with the account of the organization. In order to create a contact, we also need to use `rev-orgs.list` API to get the default [workspace](https://docs.devrev.ai/product/grow#-workspace)(rev org) created at with the account. We will be creating a contact under the workspace(rev org) of the account as contacts(rev-users) cannot exist individually without being assoiciated with a workspace(rev org).
