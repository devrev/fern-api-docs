In this tutorial, you'll learn how to create an account and associated contacts in DevRev with the `accounts-create` API. You can also [create accounts manually in the DevRev app](https://docs.devrev.ai/product/customers#add-new-customers).
An account in DevRev represents an organization or a customer.
...
Get the default rev-org created along with the account using `rev-orgs.list` API. 
    - Make a GET/POST request to "https://api.devrev.ai/rev-orgs.list" using cURL with the `account` filter. Use the `external_ref` from the previous response as an `account` filter.
    ```bash
