...
The `rev-users.create` API is used to create [contacts](https://docs.devrev.ai/product/grow#-contact) which represents an individual user associated with the account of the organization. In order to create a contact, we also need to use `rev-orgs.list` API with `accounts` filter to get the default [workspace](https://docs.devrev.ai/product/grow#-workspace)(rev org) created with the account. We will be creating a contact under the workspace(rev org) of the account as contacts(rev users) cannot exist individually without being assoiciated with a workspace(rev org).
...
3. Check **Customers > Accounts** in the DevRev app to confirm that that account was created.
...
In this tutorial, you learned to create an account and a contact associated with it using  `accounts.create`, `rev-users.create` and, `rev-orgs.list` APIs and learned how to apply `account` filter to get the required information only. You can now create multiple contacts under one account using `rev-users.create` API.