...
accTitle: Link concept
accDescr: A link has a forward and a backward name. The example shows a ticket that is dependent on an issue, and the backward name "is dependency of" is shown in the reverse direction.
...
tkt -- "&nbsp; is dependent on &nbsp;" --> iss
iss -- "&nbsp; is dependency of &nbsp;" --> tkt
```
...
<Tabs>
  ...
  | ticket | is parent of | ticket |
  | ticket | is merged into | ticket |
  | ticket | is dependent on | issue |

  <Callout intent="note">
  Only ticket → issue is allowed for "is dependent on". Creating the link in reverse (issue → ticket) is not possible; but we can swap the source and target objects to achieve the same result.
  </Callout>
  ...
  <Callout intent="note">
  Only issue → task is allowed for "is parent of". task cannot be linked as a parent of an issue.
  </Callout>
