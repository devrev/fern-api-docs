...
A link has a forward and a backward name. The example below shows an issue that is dependent on a ticket; the backward name is shown in the reverse direction.
...
iss((ISS-1))
tkt((TKT-1))
iss -- "is dependent on" --> tkt
tkt -- "is dependency of" --> iss
```
...
<Tabs>
  ...
  <Callout intent="note">
- Only ticket → issue links are allowed for "is dependent on".
- Links cannot be created in reverse (issue → ticket); the UI automatically flips them.

  </Tab>

  <Tab title="From an issue">

  ```mermaid Links from an issue
  flowchart LR
  %%{init: {
      'theme': 'base',
      'themeVariables': {
  ...
  task[task]
  code[code change]
  src -- "is related to" --> i2
  src -- "is duplicate of" --> i2
  src -- "is parent of" --> i2
  src -- "is parent of" --> task
  src -- "is dependent on" --> i2
  src -- "is dependent on" --> code
  ```
