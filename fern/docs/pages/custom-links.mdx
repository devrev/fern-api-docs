...
<Callout intent="note">
Only ticket → issue is allowed for creating a link with "is dependent on". Creating the link in reverse (issue → ticket) is not possible; but we can swap the source and target objects to achieve the same result.
</Callout>
