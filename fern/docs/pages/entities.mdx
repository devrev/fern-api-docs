...
📄 `artifacts` [API reference](/api-reference/artifacts)
...
📄 `auth-connection` [API reference](/api-reference/auth-connection)
...
📄 `auth-tokens` [API reference](/api-reference/auth-tokens)

## Product builders and service providers: `dev-users`

Dev users belong to the organization that develops, builds, or provides products or services to customers.

📄 `dev-users` [API reference](/api-reference/dev-users)
...
📄 `parts` [API reference](/api-reference/parts)

## Workspaces: `rev-orgs`

Workspaces (or rev orgs) are the organizations that your customers belong to. A workspace offers more granularity than accounts to define the instances of your product. Users can be mapped to track usage per workspace.

For more information about workspaces, refer to [Customers](https://docs.devrev.ai/product/customers).

📄 `rev-orgs` [API reference](/api-reference/rev-orgs)
...
📄 `tags` [API reference](/api-reference/tags)
...
📄 `timeline-entries` [API reference](/api-reference/timeline-entries)
...
📄 `works` [API reference](/api-reference/works)
