...
description: "A step-by-step guide to using DevRev's asynchronous API for agent execution"
---

# DevRev Agents Async API Cookbook

This cookbook provides a step-by-step guide to implementing the asynchronous API for DevRev agents. It assumes you've already created an agent using the DevRev agent builder platform.
...
The DevRev Async API enables you to execute agents without timeout concerns. The execution occurs as a temporal workflow that sends events to you via webhooks. This approach ensures:
...
- `client_metadata`: Optional JSON data that will be returned verbatim in webhook events
