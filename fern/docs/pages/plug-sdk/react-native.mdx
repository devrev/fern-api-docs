...
|`DevRev.stopRecording()`    | End the session recording and uploads it to the portal. |
|`DevRev.pauseRecording()`   | Pause the ongoing session recording.                     |
|`DevRev.resumeRecording()`  | Resume a paused session recording.                       |
|`DevRev.processAllOnDemandSessions()`  | End the ongoing session recording and uploads all offline sessions on demand, including the current one.|
