...
You can use the getSessionDetails method to fetch the session id and tab id of an currently ongoing session.These details can then be passed across different domains to maintain the journey as a single, continuous session.
