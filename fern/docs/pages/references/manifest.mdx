...
uild configuration is used to configure the build process of the Snap-In. For e.g. It can be used to set the environment variables used during the build process. This is useful if the snap-in requires libraries that
...
For further information, see [Build Config](/snap-in-development/references/build-config.mdx).
