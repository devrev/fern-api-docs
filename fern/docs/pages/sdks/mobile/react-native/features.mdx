...
const YourComponent = () => {
  ...
  useEffect(() => {
    const id = findNodeHandle(sensitiveViewRef.current);
    if (id) DevRev.markSensitiveViews([id]);
  }, []);
...
const YourComponent = () => {
  ...
  useEffect(() => {
    const id = findNodeHandle(viewRef.current);
    if (id) DevRev.unmarkSensitiveViews([id]);
  }, []);

  return (
    <View ref={viewRef}>
      {/* Insensitive content goes here */}
    </View>
  );
};
```
